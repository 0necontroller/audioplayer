...
class H5AudioPlayer extends Component<PlayerProps> {
	...
	handleClickRewind = (): void => {
		const { progressJumpSteps, progressJumpStep = 10 } = this.props;
		...
		if (
			...
			try {
				...
				return (
					...
					this.props.onChangeCurrentTimeError(err as Error)
				);
			}
		}
		...
		if (this.props.hasDefaultKeyBindings ?? true) {
			switch (e.key) {
				...
				case 'ArrowUp':
					...
					this.setJumpVolume(this.props.volumeJumpStep ?? 0.1);
					break;
				case 'ArrowDown':
					e.preventDefault(); // Prevent scrolling page by pressing arrow key
					this.setJumpVolume(-(this.props.volumeJumpStep ?? 0.1));
	...
	componentDidMount(): void {
		...
		audio.addEventListener(
			...
			}, this.props.listenInterval || 1000)
		);
